...
contract AmmRouter01 is IAmmRouter01 {
        ...
        returns (uint amountToken, uint amountETH, uint liquidity)
    {
        ...
        if (msg.value > amountETH)
            payable(msg.sender).sendValue(msg.value - amountETH); // refund dust eth, if any
    }
    ...
    function _swap(
        ...
        for (uint i; i < path.length - 1; i++) {
            ...
            address to = i < path.length - 2
                ? AmmLibrary.pairFor(factory, output, path[i + 2])
                : _to;
        ...
        returns (uint[] memory amounts)
    {
        ...
        if (msg.value > amounts[0])
            payable(msg.sender).sendValue(msg.value - amounts[0]); // refund dust eth, if any
    }
    ...
    function getAmountOut(
        ...
        return
            AmmLibrary.getAmountOut(factory, amountIn, reserveIn, reserveOut);
    }
    ...
    function getAmountIn(
        ...
        return
            AmmLibrary.getAmountIn(factory, amountOut, reserveIn, reserveOut);
    }

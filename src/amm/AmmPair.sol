...
contract AmmPair is IAmmPair, AmmERC20 {
    ...
    // KRChange fee structure
    // if fee is on, mint liquidity approximately equal to half the fees.
    function _mintFee(
        uint112 _reserve0,
        uint112 _reserve1
    ...
    function swap(
        ...
        uint feeRateBips = IAmmFactory(factory).feeRateBips();
        {
            // scope for reserve{0,1}Adjusted, avoids stack too deep errors
            uint balance0Adjusted = (
                balance0.mul(10000).sub(amount0In.mul(feeRateBips))
            );
            uint balance1Adjusted = (
                balance1.mul(10000).sub(amount1In.mul(feeRateBips))
            );
            require(
